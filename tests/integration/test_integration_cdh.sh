...
testClusterCreate(){
	...
	if [ "$DO_INTEGRATION_TEST" = true ]; then
		...
		declare -a ARR_RESULT=($(orka create cdh_integration_test 2 4 4096 5 2 4096 10 standard escience.grnet.gr 2 128 --use_hadoop_image Hadoop_2\.6\.0\-Cloudera\-CDH\-5\.4\.2))
...
testHDFSrunPI(){
	if [ "$DO_INTEGRATION_TEST" = true ]; then
		...
		'/usr/lib/hadoop/bin/hadoop jar /usr/lib/hadoop-mapreduce/hadoop-mapreduce-examples.jar pi 2 10000' 2>&1 | tee _tmp.txt
...
testHDFSwordcount(){
	if [ "$DO_INTEGRATION_TEST" = true ]; then
		...
		ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $ROOTHOST \
		'/usr/lib/hadoop/bin/hadoop jar /usr/lib/hadoop-mapreduce/hadoop-mapreduce-examples.jar wordcount LICENSE.txt out_wordcount' 2>&1 | tee _tmp.txt
		ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $ROOTHOST \
...
testHDFSteragen(){
	if [ "$DO_INTEGRATION_TEST" = true ]; then
		ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $ROOTHOST \
		'/usr/lib/hadoop/bin/hadoop jar /usr/lib/hadoop-mapreduce/hadoop-mapreduce-examples.jar teragen 2684354 out_teragen' 2>&1 | tee _tmp.txt
		ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $ROOTHOST \
