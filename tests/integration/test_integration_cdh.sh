...
testClusterCreate(){
	...
	if [ "$DO_INTEGRATION_TEST" = "$THIS_TEST" ]; then
		...
		( $(orka create cdh_integration_test 2 4 4096 5 2 4096 10 standard escience.grnet.gr --use_hadoop_image Hadoop_2\.6\.0\-Cloudera\-CDH\-5\.4\.2 >_tmp.txt 2> /dev/null) ) & keepAlive $! " Working"
