...
testClusterCreate(){
	...
	if [ "$DO_INTEGRATION_TEST" = "$THIS_TEST" ] || [ "$FULL_TESTSUITE" = "true" ]; then
		...
		( $(orka create ecosystem_integration_test 3 4 6144 10 4 6144 10 standard ${OKEANOS_PROJECT} --image Ecosystem\-Hue\-Debian\-8\.0 >_tmp.txt 2> /dev/null) ) & keepAlive $! " Working"
