...
testClusterCreate(){
	...
	if [ "$DO_INTEGRATION_TEST" = "$THIS_TEST" ]; then
		...
		( $(orka create ecosystem_integration_test 2 4 6144 10 4 6144 10 standard escience.grnet.gr --use_hadoop_image Ecosystem\-Hue\-Debian\-8\.0 >_tmp.txt 2> /dev/null) ) & keepAlive $! " Working"
