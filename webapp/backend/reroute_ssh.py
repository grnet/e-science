...
def start_vre_script(server_ip, password, admin_password, vre_image, admin_email):
    ...
    command = ". DSpace-5.3.sh {0} {1}".format(admin_password, admin_email)
...
def start_vre(server_ip, password, admin_password, vre_image, admin_email):
    ...
    if vre_image['image'] == 'dspace':
        vre_commands = ["cd;systemctl stop docker-*.scope;","/usr/bin/docker exec -d dspace sv stop tomcat8",
                        ...
                        "/usr/bin/docker exec -d dspace sv start tomcat8",vre_image['update_password'].format(admin_password),
