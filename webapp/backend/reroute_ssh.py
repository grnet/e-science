...
def start_vre(server_ip, password, token, vre_image):
    ...
    command = "cd;systemctl stop docker-*.scope;myvar=$(docker inspect {0} | grep \"Id\" | sed 's/[\" ,:]//g' | sed 's/Id//g'); cd /var/lib/docker/containers/$myvar;/etc/init.d/docker stop;find . -name config.json -exec sed -i 's/{1}/{2}/g' {{}} +;"\
    ...
    ";/usr/bin/docker start {4}; ".format(vre_image['db_name'], vre_image['default_password'], token, vre_image['update_password'].format(token), vre_image['image'])
