...
class OrkaImage(object):
    ...
    def create(self):
        ...
        sleep(60) # For VM to be pingable
        logging.log(REPORT," ~okeanos VM created, installing python...")
        subprocess.call("ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@{0} 'apt-get update && apt-get install -y python;'".format(self.server_ip),shell=True)
        self.create_ansible_hosts()
